...
typedef struct{
	POINT p;
	char jenisPetak;
	char kepemilikanPetak;// Player R dan Player B, R atau B
	unitaddress u;
} elmtPeta;
...
#define infoJPetak(M,i,j) ((M).Mem[(i)][(j)]).jenisPetak
#define infomilik(M,i,j) ((M).Mem[(i)][(j)]).kepemilikanPetak
#define infounitpetak(M,i,j) ((M).Mem[(i)][(j)]).u
